...
public class ColorManagementController {
    ...
    public ResponseEntity saveColor(@RequestBody Color color){
        return(colorManagementService.saveColor(color)) ? ResponseEntity.ok().build() : ResponseEntity.unprocessableEntity().build();
    }
    ...
    {
        List<Color> colors = colorManagementService.getAllColors(pageNumber,pageSize);
    ...
    public ResponseEntity getColor(@PathVariable long colorId){
        ...
        return (color.isPresent()) ? ResponseEntity.ok(color) : ResponseEntity.unprocessableEntity().build();
    }

    @PutMapping("/update-color")
    public ResponseEntity updateColor(@RequestBody Color color){
        return (colorManagementService.updateColor(color)) ? ResponseEntity.ok().build() : ResponseEntity.unprocessableEntity().build();
    }

    @DeleteMapping("/delete-color/{colorId}")
    public ResponseEntity deleteColor(@PathVariable long colorId){
        return (colorManagementService.deleteColor(colorId)) ? ResponseEntity.ok().build() : ResponseEntity.unprocessableEntity().build();
    }
